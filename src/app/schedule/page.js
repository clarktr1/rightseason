      ...
      <section className="max-w-4xl mx-auto px-6 py-16">
        ...
        >
          ...
          <p className="text-xl text-amber-700 max-w-3xl mx-auto leading-relaxed mb-8">
            Follow our mobile kitchen around town! Check out our schedule to see where 
            we'll be serving up fresh, delicious meals next.
          </p>
        </motion.div>
        ...
        {upcomingEvents.length === 0 ? (
          <div className="text-center py-16 bg-white rounded-3xl shadow-lg">
            ...
            <p className="text-amber-700 max-w-md mx-auto">
              We&apos;re planning our next delicious stops. Check back soon or call
              us for updates!
            </p>
          </div>
        ) : (
          <ul className="space-y-8">
            {upcomingEvents.map(({ id, name, address, time, dateNumber, dateMonth }) => (
              <motion.li
                key={id}
                initial={{ y: -30, opacity: 0 }}
                animate={{ y: 0, opacity: 1 }}
                  ...
                  <div className="flex justify-between items-start mb-4">
                    ...
                    <div className="text-center">
                      <div className="bg-white/20 backdrop-blur-sm rounded-lg p-3">
                        ...
                        <div className="text-sm font-bold text-amber-100">{dateMonth}</div>
                      </div>
                    </div>
                  </div>
                </div>
